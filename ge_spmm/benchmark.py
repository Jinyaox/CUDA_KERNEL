...
"""the DIY is the custom function for the ge-SPMM"""
def diy(m,x): #m is a CSR Tensor and x is a dense tensor
    return spmm.csr_spmm (m.crow_indices().type(torch.int32),
                     m.col_indices().type(torch.int32),
                     m.values().type(torch.float32),
                     x.type(torch.float32))



"""start of the main, origional benchmark script"""
...
def correctness(dataset):
    ...
    m_diy = mat.to_torch_sparse_csr_tensor()
...
def timing(dataset):
    ...
    ts = torch.tensor([t1, t2, t3, t4, t5])
